...
public class MemberService { //클래스
	...
	public void displayMenu() { //메뉴 화면 출력 기능
		...
		do {
			...
			case 4 :int result =updateMember(); //회원 정보 수정 메소드 수행 후 반환되는 결과를 result 변수에 저장
	...
	public String signUp() {// This method must return a result of type String
		...
		if(memberPw.equals(memberPw2)) {
			...
			return "회원가입 성공";
			//메소드를 호출한 곳으로 "회원 가입 성공" 반환

		}else {
			return "회원가입 실패";
			//메소드를 호출한 곳으로 " 회원 가입 실패" 반환
		}
	...
	public String login() {
		...
		if(	memberInfo.getMemberId().equals(memberId) && memberInfo.getMemberPw().equals(memberPw)){
			//입력받은 memberId와 필드에서 memberInfo가 참조중인 Member 객체의 memberId가 같은지 확인

			//회원가입한 회원의 정보를 loginMember;에 저장
			// -> 어떤 회원이 로그인 했는지 구분 가
			loginMember= memberInfo;
			//참조형 = Member객체 주소 (앝은 복사)
	...
	// 이름, 나이 입력 받아서
	// 로그인이 되어있으면 수정
	//+ 로그인한 회원의 비밀번호와 입력한 비밀번호가 일치하면 수정
	public String editMember() {
		if(loginMember==null) {
			...
			if(memberPw.equals(loginMember.getMemberPw())) {
				loginMember.setMemberName(editName);
				loginMember.setMemberAge(editAge);
				return "수정 완료";
	...
	public int updateMember() { //int로 반환받는법
		System.out.println("\n****회원 정보 수정 ****");
