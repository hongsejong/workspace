...
public class ConditionExample { // 예제 작성용
	...
	public void ex1() {
		...
		//입력된 정수가 양수인지 아닌지 검사
		if(input > 0) { // input이 0보다 커서 조건식이 true 인 경우 수행
	...
	public void ex8() {
		...
		if(age <0 || age > 100) {
			...
			if(k<0 || k > 250.0) {
				...
				if(age<12 && k >=140.0) { //나이 X,키 O
					result = "키는 적절하나 나이가 적절치 않음";
				}else if(age >=12 && k <140.0){ //나이 O, 키X
					result = "나이는 적절하나, 키가 적절치 않음";
				}else if(age <12 && k <140.0) { //나이 X, 키 X
