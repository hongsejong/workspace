...
--> 왜? 변경하지 않으면 c##을 사용자명 앞에 붙여야함.. 번거로움
...
CREATE USER hsj_sample  IDENTIFIED BY sample1234;

--2.생성된 계정에 접속하기 위한 접속방법 추가

-- -> 데이터베이스 -> 새 데이터베이스 연결
--ORA-01045: 사용자 HSJ_SAMPLE는 CREATE SESSION 권한을 가지고있지 않아서 로그인 실패

--3. [SYS 관리자 계정] 생성된 샘플 계정에 DB 접속 권한 부여

GRANT CREATE SESSION TO hsj_sample;
...
GRANT CREATE TABLE TO hsj_sample;

ALTER USER hsj_sample DEFAULT TABLESPACE SYSTEM QUOTA UNLIMITED ON SYSTEM;
...
--ROLE : 권한의 묶음
-->묶어둔 권한(RLOE)을 특정 계정에 부여 --> 해당 계정은 특정 권한을 가진 역할을 갖게됨
...
--2) RESOURCE : DB사용을 위한 기본 객체 생성 권한 묶음
...
--[SYS 관리자 계정]
GRANT CONNECT, RESOURCE TO hsj_sample;

----------------------------------------------------------

--객체 권한
-->hsj_sample 계정에서 KH_HSJ 계정의 EMPLOYEE 테이블 조회

--1.[샘플 계정] KH_HSJ 계정의 EMPLOYEE테이블 조회
SELECT * FROM KH_HSJ.EMPLOYEE; --> EMPLOYEE 테이블(객체) 조회 권한 없음


--2. [KH 계정] 샘플 계정에 EMPLOYEE 테이블 조회 권한 부여

GRANT SELECT ON EMPLOYEE TO hsj_sample;

--3. [샘플 계정] 다시 EMPLOYEE 테이블 조회
SELECT * FROM KH_HSJ.EMPLOYEE;

--4. [KH 계정] 부여했던 SELECT 권한 회수

REVOKE SELECT ON EMPLOYEE FROM hsj_sample;

--5. [샘플 계정] 다시 EMPLOYEE 테이블 조회
SELECT * FROM KH_HSJ.EMPLOYEE; --> 조회 안됨
