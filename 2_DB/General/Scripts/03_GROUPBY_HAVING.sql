...
--여러 개의 결과 값을 산출하기 위해 그룹 함수가 적용된 그룹의 기준을 GROUP BY 절에 기술

--EMPLOYEE 테이블에서 부서코드, 부서(그룹) 별 급여 합계
...
--EMPLOYEE 테이블에서
--성별과 성별 별 급여 평균(정수처리), 급여 합계,인원수를 조회하고
--인원 수로 내림차순 정렬
SELECT DECODE(SUBSTR(EMP_NO,8,1),'1','남자','2','여자') 성별,FLOOR(AVG(SALARY))"급여 평균",SUM(SALARY)"급여 합계",COUNT(*) "인원 수"
...
--> WHERE절은 각 컬럼 값에 대한 조건(SELECT문 해석 순서!!)
...
--EMPOYEE 테이블에서 부서 별로 급여 등급이 같은 직원 수를 조회하고
...
-- 부서별 그룹의 급여 합계중 9백만원을 초과하는 부서코드와 급여 합계 조회
--부서 코드 순으로 정렬
