...
WHERE BOARD_NO = 1300;
--댓글등록
INSERT INTO "COMMENT" VALUES(SEQ_COMMENT_NO.NEXTVAL, '댓글내용',DEFAULT,DEFAULT,DEFAULT,'보드넘버','멤버넘버');
--보드상태 업뎃
UPDATE BOARD WET
BOARD_STATUS ='Y'
WHERE BOARD_NO='';
...
SELECT * FROM REPORT ;
--비밀번호 user01세팅
UPDATE MEMBER SET MEMBER_PW = '$2a$10$As1ZxP4sxts4fI5lhHwtYOrW6ZvEBLZNeIMBc14hT6nAY3tjP9Y3W';
...
--신고 인서트
INSERT INTO REPORT VALUES(SEQ_REPORT_NO.NEXTVAL, '멤버넘버','보드넘버','컨텐츠','타이틀','타입(12345)',DEFAULT);
...
SELECT * FROM USER_BAN
WHERE BAN_END IS NOT NULL AND BAN_COUNT =1;


SELECT * FROM "MEMBER" JOIN USER_BAN USING(MEMBER_NO)
WHERE BAN_END IS NOT NULL AND BAN_COUNT =1
AND BAN_END<SYSDATE;
...
SELECT COUNT(*) FROM BOARD WHERE (TRUNC(B_CREATE_DATE) = TRUNC(SYSDATE)) AND BOARD_CODE =6;


--어제꺼랑 비교하기
SELECT 
  (SELECT COUNT(*) FROM BOARD WHERE TRUNC(B_CREATE_DATE) = TRUNC(SYSDATE))
  - (SELECT COUNT(*) FROM BOARD WHERE TRUNC(B_CREATE_DATE) = TRUNC(SYSDATE - 1)) AS YESCOUNT
FROM dual;
SELECT * FROM USER_BAN ;
...
SELECT 
  (SELECT COUNT(*) FROM BOARD 
   WHERE TRUNC(B_CREATE_DATE) = TRUNC(SYSDATE)
     AND BOARD_CODE = 6)
  - (SELECT COUNT(*) FROM BOARD 
     WHERE TRUNC(B_CREATE_DATE) = TRUNC(SYSDATE - 1)
       AND BOARD_CODE = !6) AS YESCOUNT
FROM dual;

--이건 투차안되잇어서 이상하게나옴
SELECT TRUNC(B_CREATE_DATE) AS inquiry_date, COUNT(*) AS inquiryCount
FROM "BOARD" 
WHERE BOARD_CODE = 6
  AND TRUNC(B_CREATE_DATE) BETWEEN TRUNC(SYSDATE) - 9 AND TRUNC(SYSDATE)
GROUP BY TRUNC(B_CREATE_DATE)
