...
SELECT S.STUDENT_NO  ,TO_CHAR(POINT,'FM9.00')  
...
FROM TB_CLASS
JOIN TB_CLASS_PROFESSOR ON TB_CLASS.CLASS_NO = TB_CLASS_PROFESSOR.CLASS_NO
JOIN TB_PROFESSOR ON TB_CLASS_PROFESSOR.PROFESSOR_NO = TB_PROFESSOR.PROFESSOR_NO
ORDER BY TB_CLASS.CLASS_NAME;
...
SELECT STUDENT_NO, STUDENT_NAME
FROM (SELECT STUDENT_NO, STUDENT_NAME, RANK() OVER(ORDER BY AVG(POINT) DESC) 랭킹
FROM TB_STUDENT
JOIN TB_GRADE USING(STUDENT_NO)
WHERE DEPARTMENT_NO = (SELECT DEPARTMENT_NO
FROM TB_DEPARTMENT
