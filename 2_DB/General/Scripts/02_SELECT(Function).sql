...
/*망한듯한거
 * SELECT PROFESSOR_NAME 교수이름 ,ABS(FLOOR( (SYSDATE -TO_DATE(SUBSTR(PROFESSOR_SSN,1,2), 'RR'))/365)) 나이
 
FROM TB_PROFESSOR 
WHERE SUBSTR(PROFESSOR_SSN,8,1)=1
ORDER BY FLOOR( (SYSDATE -TO_DATE(SUBSTR(PROFESSOR_SSN,1,4), 'YY')));
*/
--4번
...
--13번
--없는값 반환하게 DECODE해서 억지로 세개함
--SELECT DEPARTMENT_NO 학과코드명, COUNT(DECODE(ABSENCE_YN,'Y','Y','NO','N')) 휴학생
--FROM TB_STUDENT
--GROUP BY DEPARTMENT_NO 
--ORDER BY 학과코드명;

--다른방법
SELECT DEPARTMENT_NO 학과코드명, SUM ((DECODE(ABSENCE_YN,'Y','1','N','0'))) 휴학생
FROM TB_STUDENT
GROUP BY DEPARTMENT_NO 
ORDER BY 학과코드명;
/*2 안나오는거
...
--15번
SELECT    SUBSTR(TERM_NO,1,4) 년도 ,SUBSTR(TERM_NO,5,2) 학기 ,ROUND( AVG(POINT) ,1) AS "년도 별 평점"
