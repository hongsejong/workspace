...
--서브쿼리를 이용해서 DEPARTMENT 테이블을 복사 -- NOT NULL 제약조건만 복사됨
...
--DEPARTMENT_COPY 테이블에 DEPT_TITLE 컬럼에 UNIQUE 제약조건 추가(제약조건명 : DEPT_TITLE_U)
...
ALTER TABLE DEPARTMENT_COPY ADD CONSTRAINT LOCATION_ID_CHK 
...
--NOT NULL은 테이블 레벨에서만 지정가능 (컬럼레벨X)

------------------------------------
-- DEPARTMENT_COPY 에 추가한 제약 조건 중 PK 빼고 모두 삭제

-- 제약조건 삭제 : ALTER TABLE 테이블명
--                 DROP CONSTRAINT 제약조건명;
ALTER TABLE DEPARTMENT_COPY DROP CONSTRAINT DEPT_TITLE_U ;
ALTER TABLE DEPARTMENT_COPY DROP CONSTRAINT LOCATION_ID_CHK ;
ALTER TABLE DEPARTMENT_COPY MODIFY DEPT_TITLE CONSTRAINT SYS_C007489 NULL; 
--ALTER TABLE DEPARTMENT_COPY DROP CONSTRAINT SYS_C007488 ;
...
SELECT * FROM DEPARTMENT_COPY ; --> 기본 값을 변경했지만 기존에 저장된 값이 변경되진 않음(앞으로 추가되는 DEFAULT 변경)

--컬럼값 변경시 UPDATE 사용!
UPDATE DEPARTMENT_COPY SET LO_NAME ='대한민국';
UPDATE DEPARTMENT_COPY SET LO_NAME =DEFAULT; -- DEFAULT 를 이용한 데이터 수정 방법
...
SELECT *FROM DEPARTMENT_COPY ;--컬럼 3개 확인.


--DEPT_TITLE, LOCATION_ID 컬럼도 삭제
ALTER TABLE DEPARTMENT_COPY DROP COLUMN DEPT_TITLE;
ALTER TABLE DEPARTMENT_COPY DROP COLUMN LOCATION_ID;

SELECT *FROM DEPARTMENT_COPY ;--컬럼 1개 확인.


--(컬럼 삭제 시 문제점)
--마지막 남은 DEPT_ID컬럼 삭제
ALTER TABLE DEPARTMENT_COPY DROP COLUMN DEPT_ID; --삭제 실패
...
ROLLBACK;
SELECT *FROM DEPARTMENT_COPY ;--'D0'가 사라지지 않음
--컬럼 수정 : ALTER TABLE 테이블명 MODIFY 컬럼명 데이터타입;   (데이터 타입 변경)
...
--> 제약 조건까지 모두 삭제
--원래는 FK 제약조건으로 인해 삭제가 불가능하지만,
--제약 조건을 없애버려서 FK 관계를 해제
